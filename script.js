...
// Función para crear el fondo animado del header
function createAnimatedHeader() {
  ...
  header.innerHTML = `
        <div class="header-background" id="mathBackground">
            ${mathSymbols
              .map(
                (symbol, index) => `<div class="floating-math">${symbol}</div>`
              )
              .join("")}
        </div>
...
// Función para actualizar botones con iconos
function setupButtonIcons() {
  ...
  checkBtn.innerHTML =
    '<i class="fas fa-check-circle"></i> ' + checkBtn.textContent;
  hintBtn.innerHTML = '<i class="fas fa-lightbulb"></i> ' + hintBtn.textContent;
  nextBtn.innerHTML =
    '<i class="fas fa-arrow-right"></i> ' + nextBtn.textContent;
  customBtn.innerHTML =
    '<i class="fas fa-pencil-alt"></i> ' + customBtn.textContent;

  // Agregar sonidos a los botones
  [checkBtn, hintBtn, nextBtn, customBtn].forEach((button) => {
...
function checkFactorization() {
  ...
  if (
    ...
    feedbackElement.textContent =
      "Por favor, ingresa números válidos (mayores a 0)";
    feedbackElement.className = "feedback incorrect";
    playSound("incorrect"); // SONIDO DE ERROR
    return;
  }
  ...
  if (
    (userSign1 === currentEquation.sign1 &&
      userSign2 === currentEquation.sign2 &&
      userFactor1 === currentEquation.factor1 &&
      userFactor2 === currentEquation.factor2) ||
    (userSign1 === currentEquation.sign2 &&
      userSign2 === currentEquation.sign1 &&
      userFactor1 === currentEquation.factor2 &&
      userFactor2 === currentEquation.factor1)
  ) {
    feedbackElement.textContent =
      "¡Correcto! La factorización es correcta. Ahora encuentra las raíces.";
    feedbackElement.className = "feedback correct";
    playSound("correct"); // SONIDO DE CORRECTO
    ...
    const factor1 =
      userSign1 === currentEquation.sign1 ? userFactor1 : userFactor2;
    const factor2 =
      userSign2 === currentEquation.sign2 ? userFactor2 : userFactor1;
...
function checkRoots() {
  ...
  if (
    (Math.abs(userRoot1 - currentEquation.root1) < 0.001 &&
      Math.abs(userRoot2 - currentEquation.root2) < 0.001) ||
    (Math.abs(userRoot1 - currentEquation.root2) < 0.001 &&
      Math.abs(userRoot2 - currentEquation.root1) < 0.001)
  ) {
    feedbackElement.textContent =
      "¡Excelente! Has encontrado correctamente las raíces de la ecuación.";
    feedbackElement.className = "feedback correct";
...
function isFactorizable(a, b, c) {
  ...
  discriminantInfoText += `D = b² - 4ac = (${b})² - 4(${a})(${c}) = ${
    b * b
  } - ${4 * a * c} = ${discriminant}<br>`;
  ...
  if (discriminant < 0) {
    ...
    return {
      factorizable: false,
      reason:
        "El discriminante es negativo, por lo que la ecuación no tiene soluciones reales.",
  ...
  if (sqrtDiscriminant !== Math.floor(sqrtDiscriminant)) {
    ...
    return {
      factorizable: false,
      reason:
        "El discriminante no es un cuadrado perfecto, por lo que la ecuación no se puede factorizar con números enteros.",
      discriminantInfo: discriminantInfoText,
    };
  }
  ...
  if (factor1 !== Math.floor(factor1) || factor2 !== Math.floor(factor2)) {
    ...
    return {
      factorizable: false,
      reason:
        "Los factores no son números enteros, por lo que la ecuación no se puede factorizar con coeficientes enteros.",
      discriminantInfo: discriminantInfoText,
    };
  }
...
function nextEquation() {
  playSound("next"); // SONIDO DE SIGUIENTE
  if (isCustomEquation) {
